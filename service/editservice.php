...
<div id="wrapper">
	...
	$tampil = "select * from service where kodeservice='$kodeservice'";
	$hasil = mysql_query($tampil);
	echo "<form action=editservicedua.php method=POST>";
	echo "<table width=95% border='1'>";	
	while($data = mysql_fetch_array($hasil)){
		echo "<tr><td width=125px>kodeservice </td>
		<td><input type=text class=pendek name=kodeservice value='$data[kodeservice]'></td></tr>";
		echo "<tr><td>tglservice </td>
		<td><input type=text class=sedang name=tglservice value='$data[tglservice]'></td></tr>";
		echo "<tr><td>biayaservice </td>
		<td> <input type=text class=pendek name=biayaservice value='$data[biayaservice]'></td></tr>";
		echo "<tr><td>idjenisservice </td>
		<td> <input type=text class=sedang name=idjenisservice value='$data[idjenisservice]'></td></tr>";
	}
	echo "</table>";
