...
<div id="wrapper">
	...
	$tampil = "select * from pemilik where kodepemilik='$kodepemilik'";
	$hasil = mysql_query($tampil);
	echo "<form action=editpemilikdua.php method=POST>";
	echo "<table width=95% border='1'>";	
	while($data = mysql_fetch_array($hasil)){
		echo "<tr><td width=125px>kodepeilik </td>
		<td><input type=text class=pendek name=kodepemilik value='$data[kodepemilik]'></td></tr>";
		echo "<tr><td>nmpemilik </td>
		<td><input type=text class=sedang name=nmpemilik value='$data[nmpemilik]'></td></tr>";
		echo "<tr><td>almtpemilik </td>
		<td> <input type=text class=pendek name=almtpemilik value='$data[almtpemilik]'></td></tr>";
		echo "<tr><td>telppemilik </td>
		<td> <input type=text class=sedang name=telppemilik value='$data[telppemilik]'></td></tr>";
