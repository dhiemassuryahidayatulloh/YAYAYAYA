...
<div id="wrapper">
	...
	$tampil = "select * from setoran where nosetoran='$nosetoran'";
	$hasil = mysql_query($tampil);
	echo "<form action=editsetorandua.php method=POST>";
	echo "<table width=95% border='1'>";	
	while($data = mysql_fetch_array($hasil)){
		echo "<tr><td width=125px>nosetoran </td>
		<td><input type=text class=pendek name=nosetoran value='$data[nosetoran]'></td></tr>";
		echo "<tr><td>tglsetoran </td>
		<td><input type=text class=sedang name=tglsetoran value='$data[tglsetoran]'></td></tr>";
		echo "<tr><td>jumlah </td>
		<td> <input type=text class=pendek name=jumlah value='$data[jumlah]'></td></tr>";
	}
	echo "</table>";
