...
<div id="wrapper">
	...
	<div id="rightContent">
	<h3>Input setoran</h3>
	<form action="inputsetoran.php" method="post">
	<table width="95%">
		<tr><td width="125px"><b>nosetoran</b></td><td><input type="text" name="nosetoran" class="pendek"></td></tr>
		<tr><td width="125px"><b>tglsetoran</b></td><td><input type="text" name="tglsetoran" class="sedang"></td></tr>
		<tr><td width="125px"><b>jumlah</b></td><td><input type="text" name="jumlah" class="panjang"></td></tr>
	</table>
	...
	$jumlah = $_POST['jumlah'];
	$cek_nosetoran = mysql_num_rows(mysql_query("select * from setoran where nosetoran='$nosetoran'"));
		
	if((empty($nosetoran)) || (empty($tglsetoran)) ||  (empty($jumlah))){
		echo "Maaf, data tidak boleh kosong.";
	} elseif ($cek_nosetoran > 0){
		echo "nosetoran tidak boleh ada yang sama.";
	} else {
		$input = "insert into setoran values('$nosetoran','$tglsetoran','$jumlah')";
		mysql_query($input);
		if ($input){
			echo "Data berhasil disimpan. <a href=tsetoran.php>kembali</a>";
		} else {
