...
<div id="wrapper">
	...
	$tampil = "select * from pelanggan where noktp='$noktp'";
	$hasil = mysql_query($tampil);
	echo "<form action=editpelanggandua.php method=POST>";
	echo "<table width=95% border='1'>";	
	while($data = mysql_fetch_array($hasil)){
		echo "<tr><td width=125px>noktp </td>
		<td><input type=text class=pendek name=noktp value='$data[noktp]'></td></tr>";
		echo "<tr><td>namapel </td>
		<td><input type=text class=sedang name=namapel value='$data[namapel]'></td></tr>";
		echo "<tr><td>alamatpel </td>
		<td> <input type=text class=pendek name=alamatpel value='$data[alamatpel]'></td></tr>";
		echo "<tr><td>telppel </td>
		<td> <input type=text class=sedang name=telppel value='$data[telppel]'></td></tr>";
	}
	echo "</table>";
